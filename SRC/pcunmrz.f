      SUBROUTINE PCUNMRZ( SIDE, TRANS, M, N, K, L, A, IA, JA, DESCA,
...
*        Q = H(1)' H(2)' . . . H(k)'
*
*  as returned by PCTZRZF. Q is of order M if SIDE = 'L' and of order N
*  if SIDE = 'R'.
*
*  Notes
*  =====
...
*          H(i), IA <= i <= IA+K-1, as returned by PCTZRZF in the
...
*          elementary reflectors H(i) as returned by PCTZRZF.
...
*     .. Local Scalars ..
     ...
     $                   ICOFFC, ICROW, ICTXT, IINFO, IPW, IROFFC, JAA,
     $                   JCC, LCM, LCMP, LWMIN, MI, MPC0, MQA0, MYCOL,
     $                   MYROW, NI, NPCOL, NPROW, NQ, NQC0
*     ..
*     .. Local Arrays ..
      INTEGER            IDUM1( 5 ), IDUM2( 5 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PCLARZB,
     $                   PCLARZT, PCUNMR3, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
         ...
         IDUM2( 5 ) = 17
         IF( LEFT ) THEN
            CALL PCHK2MAT( K, 5, M, 3, IA, JA, DESCA, 10, M, 3, N, 4,
     $                     IC, JC, DESCC, 15, 5, IDUM1, IDUM2, INFO )
         ELSE
            CALL PCHK2MAT( K, 5, N, 4, IA, JA, DESCA, 10, M, 3, N, 4,
     $                     IC, JC, DESCC, 15, 5, IDUM1, IDUM2, INFO )
         END IF
      END IF
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCUNMRZ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. NOTRAN ) ) THEN
         ...
         ELSE
            NI = N
         END IF
         CALL PCUNMR3( SIDE, TRANS, MI, NI, IB, L, A, IA, JA, DESCA,
     $                 TAU, C, IC, JC, DESCC, WORK, LWORK, IINFO )
      END IF
*
...
*
         CALL PCLARZT( 'Backward', 'Rowwise', L, IB, A, I, JAA, DESCA,
     $                 TAU, WORK, WORK( IPW ) )
         IF( LEFT ) THEN
*
...
*        Apply H or H'
*
         CALL PCLARZB( SIDE, TRANST, 'Backward', 'Rowwise', MI, NI, IB,
     $                 L, A, I, JAA, DESCA, WORK, C, ICC, JCC, DESCC,
     $                 WORK( IPW ) )
   10 CONTINUE
*
