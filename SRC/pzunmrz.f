      SUBROUTINE PZUNMRZ( SIDE, TRANS, M, N, K, L, A, IA, JA, DESCA,
...
*     .. External Subroutines ..
     ...
     $                   PB_TOPSET, PXERBLA, PZLARZB, PZLARZT,
     $                   PZUNMR3
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZUNMRZ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. NOTRAN ) ) THEN
         ...
         ELSE
            NI = N
         END IF
         CALL PZUNMR3( SIDE, TRANS, MI, NI, IB, L, A, IA, JA, DESCA,
...
*
         CALL PZLARZT( 'Backward', 'Rowwise', L, IB, A, I, JAA, DESCA,
     $                 TAU, WORK, WORK( IPW ) )
         IF( LEFT ) THEN
*
...
*        Apply H or H'
*
         CALL PZLARZB( SIDE, TRANST, 'Backward', 'Rowwise', MI, NI, IB,
