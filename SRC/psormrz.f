      SUBROUTINE PSORMRZ( SIDE, TRANS, M, N, K, L, A, IA, JA, DESCA,
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PSLARZB,
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSORMRZ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. NOTRAN ) ) THEN
         ...
         ELSE
            NI = N
         END IF
         CALL PSORMR3( SIDE, TRANS, MI, NI, IB, L, A, IA, JA, DESCA,
...
*
         CALL PSLARZT( 'Backward', 'Rowwise', L, IB, A, I, JAA, DESCA,
     $                 TAU, WORK, WORK( IPW ) )
         IF( LEFT ) THEN
*
...
*        Apply H or H'
*
         CALL PSLARZB( SIDE, TRANST, 'Backward', 'Rowwise', MI, NI, IB,
